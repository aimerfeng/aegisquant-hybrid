...
//!
//! Feature: aegisquant-optimizations, Property 5: L1 订单簿撮合正确�?
...
fn arb_orderbook() -> impl Strategy<Value = OrderBookSnapshot> {
    // Generate 3-5 bid levels (descending prices from 99 to 90)
    ...
    // Generate 3-5 ask levels (ascending prices from 101 to 110)
...
proptest! {
    ...
    /// Property 5: L1 订单簿撮合正确�?- Buy orders
    ///
    /// For any order and orderbook depth combination:
    /// - If order quantity <= total available, should be fully filled
    /// - If order quantity > total available, should be partially filled
    /// - Average price should match volume-weighted calculation
    ///
    /// **Validates: Requirements 5.2, 5.3, 5.4**
    #[test]
    fn prop_l1_orderbook_buy_matching(
        ...
        // Verify filled quantity matches
        prop_assert!(
        ...
        // Verify unfilled quantity matches
        prop_assert!(
        ...
        // Verify average price matches (if there were fills)
    ...
    /// Property 5: L1 订单簿撮合正确�?- Sell orders
    ///
    /// **Validates: Requirements 5.2, 5.3, 5.4**
    #[test]
    ...
    fn prop_partial_fill_when_exceeds_liquidity(
        ...
        // Create an order larger than available liquidity
        ...
        // Filled should be approximately equal to available liquidity
        prop_assert!(
    ...
    fn prop_slippage_increases_with_size(
        ...
        let fill_ratio = 1.0; // Full liquidity available
        ...
        // If both orders filled at the same level, large order should have higher avg price
